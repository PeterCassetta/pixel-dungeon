...
import com.petercassetta.pdrebalanced.effects.Fireball;
import com.petercassetta.pdrebalanced.ui.Archs;
import com.petercassetta.pdrebalanced.ui.ExitButton;
import com.petercassetta.pdrebalanced.ui.PrefsButton;

public class TitleScene extends PixelScene {

    private static final String TXT_REBALANCED	= "Rebalanced";
	...
	public void create() {
		...
		float height = title.height + 
			(PDRebalanced.landscape() ? DashboardItem.SIZE : DashboardItem.SIZE * 2);
		
		title.x = (w - title.width()) / 2;
		title.y = (h - height) / 2;

        // "Rebalanced" text
        BitmapText rebalanced = new BitmapText(TXT_REBALANCED, font15x);
        rebalanced.measure();
        rebalanced.hardlight( 0xFF4444 );
        rebalanced.x = (w - rebalanced.width()) / 2;
        rebalanced.y = title.y + title.height - rebalanced.height;
        add( rebalanced );
		...
		DashboardItem btnAbout = new DashboardItem( TXT_ABOUT, 1 ) {
			@Override
			protected void onClick() {
				PDRebalanced.switchNoFade(AboutScene.class);
			}
		};
		add( btnAbout );
		...
		DashboardItem btnPlay = new DashboardItem( TXT_PLAY, 0 ) {
			@Override
			protected void onClick() {
				PDRebalanced.switchNoFade(StartScene.class);
			}
		};
		add( btnPlay );
		...
		DashboardItem btnHighscores = new DashboardItem( TXT_HIGHSCORES, 2 ) {
			@Override
			protected void onClick() {
				PDRebalanced.switchNoFade(RankingsScene.class);
			}
		};
		add( btnHighscores );
