...
import com.petercassetta.pdrebalanced.effects.BannerSprites.Type;
...
import com.petercassetta.pdrebalanced.windows.WndChallenges;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		curClass = null;
		updateClass( HeroClass.values()[PDRebalanced.lastClass()] );
	...
	private void startNewGame() {
		...
		if (PDRebalanced.intro()) {
			PDRebalanced.intro(false);
	...
	private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( PDRebalanced.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add( new WndChallenges( PDRebalanced.challenges(), true ) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( PDRebalanced.challenges() > 0 ?
